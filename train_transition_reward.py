...
parser.add_argument('--datasets_auxiliary', type=str, nargs='*')
...
interactions_transition_file = os.path.join(save_folder, 'interactions_transition.pt')
...
dataset = utils.StateTransitionsDataset(
    hdf5_file=args.dataset, hdf5_files_auxiliary=args.datasets_auxiliary)
...
if args.interactions == 'gnn':
    interactions_transition = modules.TransitionGNN(
        input_dim=args.embedding_dim,
        hidden_dim=args.hidden_dim,
        action_dim=args.action_dim,
        num_objects=args.num_objects,
    ...
    interactions_transition = modules.AttentionV1(
...
if interactions_transition is not None:
    interactions_reward = copy.deepcopy(interactions_transition)
    interactions_transition = interactions_transition.to(device)
    interactions_reward = interactions_reward.to(device)
...
for epoch in range(1, args.epochs + 1):
    ...
    for batch_idx, data_batch in enumerate(train_loader):
        ...
        if args.decoder:
            ...
            if args.interactions == 'gnn':
                logit_transition = interactions_transition([embedding, orig_action, torch.ones_like(moving_boxes), False])[0].squeeze(dim=2)
                logit_reward = interactions_reward([embedding, orig_action, torch.ones_like(moving_boxes), False])[0].squeeze(dim=2)
                moving_boxes = torch.sigmoid(logit_transition)
                moving_boxes_reward = torch.sigmoid(logit_reward)
            elif args.interactions == 'soft':
                one_hot_action = utils.to_one_hot(orig_action, args.action_dim)
            ...
            if args.reconstruction_loss_coef > 0:
                ...
                reconstruction_loss = F.binary_cross_entropy_with_logits(reconstructed_obs, obs)
    ...
    if avg_loss < best_loss:
        ...
        if interactions_transition is not None:
            torch.save(interactions_transition.state_dict(), interactions_transition_file)
        if interactions_reward is not None:
            torch.save(interactions_reward.state_dict(), interactions_reward_file)
